...
const child = spawn(executable, args, { cwd: path.join(root, role === 'api' ? 'api' : 'web'), stdio: 'inherit', windowsHide: true, env: { ...process.env, NODE_ENV: 'production', ASPNETCORE_ENVIRONMENT: 'Production', API_URL: 'http://127.0.0.1:5180', DataDirectory: path.join(root, 'api/App_Data') } });
