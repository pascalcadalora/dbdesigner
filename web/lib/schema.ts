...
export const relationSchema = z.object({ id: z.uuid(), fromTable: z.uuid(), fromColumn: z.uuid(), toTable: z.uuid(), toColumn: z.uuid(), kind: z.enum(['one-to-many', 'one-to-one']) });
...
export function demoProject(): Project {
  ...
  const rel = (a: Table, b: Table, col: number): Relation => ({ id: uid(), fromTable: a.id, fromColumn: a.columns[0].id, toTable: b.id, toColumn: b.columns[col].id, kind: 'one-to-many' });
...
export function connect(p: Project, fromTable: string, fromColumn: string, toTable: string, toColumn: string, kind: Relation['kind']): Project {
  const next = structuredClone(p);
  if (kind === 'one-to-one') { const c = next.tables.find(t => t.id === toTable)?.columns.find(c => c.id === toColumn); if (c) c.unique = true; }
  next.relations.push({ id: uid(), fromTable, fromColumn, toTable, toColumn, kind });
...
export function junction(p: Project, left: Table, right: Table): Project {
  ...
  next.tables.push(t);
  next.relations.push({id: uid(), fromTable: left.id, fromColumn: a.id, toTable: t.id, toColumn: t.columns[0].id, kind: 'one-to-many'}, {id: uid(), fromTable: right.id, fromColumn: b.id, toTable: t.id, toColumn: t.columns[1].id, kind: 'one-to-many'});
  return validateProject(next);
}
